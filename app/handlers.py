...
async def send_text_details(callback: CallbackQuery):
    await callback.message.answer(load_text("text_work"), reply_markup=text_details_inline, parse_mode="HTML")
    await callback.answer()

@router.callback_query(F.data == "oсr_text")
async def send_osr_text(callback: CallbackQuery):
    await callback.message.answer(load_text("oсr"), reply_markup=contact_inline, parse_mode="HTML")
    await callback.answer()

@router.callback_query(F.data.in_(["design_details", "bot_details"]))
async def send_details(callback: CallbackQuery):
    await callback.message.answer(load_text(DETAILS_TEXT_MAP[callback.data]), reply_markup=contact_inline, parse_mode="HTML")
    await callback.answer()
